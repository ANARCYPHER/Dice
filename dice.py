...
def roll_dice():
	...
	# Update Total Label
	total = sd1 + sd2
	total_label.config(text=f"Rolled: {total}")
...
# Create Dice Labels
dice_label1 = Label(my_frame, text='', font=("Helvetica", 100), fg="gold")
dice_label1.grid(row=0, column=0, padx=5)
sub_dice_label1 = Label(my_frame, text="")
sub_dice_label1.grid(row=1, column=0)

dice_label2 = Label(my_frame, text='', font=("Helvetica", 100), fg="gold")
...
# Create Roll Button
my_button = Button(root, text="Roll Dice", command=roll_dice, font=("Helvetica", 30))
my_button.pack(pady=20)

# Create Totals Label
total_label = Label(root, text="", font=("Helvetica", 30), fg="red")
